			...
			this.ws.on('connection', (sc) => {
				...
				sc.on('message', (data) => {
					...
					.then(reply => {
						...
						sc.send(JSON.stringify({state: this.mstatus.state, service: this.service, realm: this.realm, track: reply}));
					});
				});
			});
...
//			});
	...
	static async init () {
		...
		try {
			...
			console.error(err);
			cntrlr.errors.MPD = 'Could not connect to the server Music Player Daemon.<br>Audio play at the server will not be possible';
	...
	//	console.log('@mpd.@\n', info)
		return info ? (info.title ? info.title : this.ttitle) : '?-?-?';
	}
...
// returned as a place holder when MPD is not running on the server
class fakeMPD {
	constructor () {
		this.service = 'fa';
	}
	sendCommand (cmd) {
		throw new Error('MPD not available');
		return '!!Server audio output is not available';
	}
	sendCommandB (cmd) {
		console.log(cmd);
		return Promise.resolve('!!Server audio output is not available');
	}
}