...
export default class NmsAdmin {
	...
	static #getStreamWithRedirects (targetUrl, maxRedirects = 5) {
		...
		return new Promise((resolve, reject) => {
			https.get(targetUrl,{headers:{'User-Agent': 'node/nodems/'+cntrlr.nmsversion}} , (response) => {
	...
	static #updateFromFile (tag, url, resp) {
		...
		writeStream.on('finish', () => {
			cntrlr.executeFile('./update.sh')
			.then(rslt2 => {
				console.log('UPDEX '+rslt2);
				resp.end(`Completed update to ${tag} ... please restart the app`);
				//cntrlr.execute('/usr/bin/systemctl restart nodems');
			})
			.catch(err => {
				console.error(String(err));
				resp.end('Update failure');
			});
		});
	...
	static #getScriptAsset (data) {
		return new Promise((resolve, reject) => {
			if (!data.assets) {
				resolve();
				return;
			}
			const writeStream = createWriteStream('update.sh');
			writeStream.on('finish', () => {
				resolve('attained asset');
			});
			NmsAdmin.#getStreamWithRedirects(data.assets[0].browser_download_url)
			.then(stream => {
				stream.pipe(writeStream);
			})
			.catch(err => {
				console.error('Error: ', err);
				reject('Failed to stream asset');
			});
		});
	}
	...
	static doUpdate (resp) {
		...
		.then(data => {
			console.log(data);
			if (data.tag_name.slice(1) > cntrlr.nmsversion) {
				NmsAdmin.#getScriptAsset(data)
				.then(()=>NmsAdmin.#updateFromFile(data.tag_name, data.zipball_url, resp));
			}
			else resp.end('No update available');
		})
		.catch(error => {
			console.error('Error: ', error);
			resp.end('Failed to get repository data')
		});
	}
